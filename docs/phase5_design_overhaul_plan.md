...
- [x] 5-G: 実装完了(実装日: 2026-07-16, ブランチ: feature/5-g-schedule-table-colors)
...
**実装メモ(2026-07-16)**:
- `buildVisibleScheduleList`の再帰関数`visit`に`groupIndex`引数を追加。depth===0(トップレベル)の項目は
  兄弟内でのインデックス(0始まり)を自身の`groupIndex`とし、depth>0の子孫は親から受け継いだ`groupIndex`を
  そのまま使う(トップレベル項目の並び順インデックスが子孫全体に伝播する)
- 新規定数`SCHEDULE_PARENT_ROW_COLORS`(薄い青系5色)を追加し、`entry.hasChildren`な行だけ
  `groupIndex % 5`で選んだ色をinline `style="background:..."`で付与。子に展開されず子を持たない
  トップレベル項目(hasChildren=false)は他の子行と同様、無色のままとした(4-Bのカンバン等とは無関係の判定)
- 検証は`buildVisibleScheduleList`をindex.htmlから抽出したテストページで、親子混在のダミーデータに対し
  `groupIndex`の割り当てと配色ロジックが仕様通りになることを確認した
...
**補足(2026-07-16)**: 5-Eの検証中、IndexedDBへ実データを投入するテスト手法(headless Chromeを
`--user-data-dir`で永続化しバックグラウンド実行する方式)を試みたが、プロセスが正常終了せずWindows側に
chrome.exeが残留する問題が発生したため中止した。以降5-E〜5-Gの検証は、対象関数をindex.htmlから
正規表現でそのまま抽出した単発の(バックグラウンド化しない)テストページを都度作成し、1回限りの
`--screenshot`呼び出しで確認する方式に統一した。今後、実データでの見た目確認が必要な場合は
IndexedDB投入の自動化は行わず、ユーザーによるブラウザでの手動確認を案内すること。

## Phase5 全体サマリー(2026-07-16)

5-A〜5-Gの全サブステップが完了した。各ステップは`feature/5-a-...`〜`feature/5-g-...`の
個別ブランチで実装し、都度developへPR・マージ済み。

**実装した内容**:
- デザイン基盤: Heroicons風アイコンスプライト(約30種)と共通ボタンCSS(`.btn`+色バリエーション)を新設し、
  ヘッダー/操作バー/モーダル/サイドパネル/行操作など、アプリ内のほぼ全ボタンに適用した
- ヘッダー・操作バーをdocs/rules指定の左右配置・並び順に変更
- 表示単位・表示列をドロップダウン/チェックボックスからセグメントコントロールに変更
- ガントチャートヘッダーを2段化し、月/四半期/年の期間バンドを上段に表示
- ガントバーに実営業日数を表示し、親はバー内・子はバー上にラベルを表示するよう変更
- スケジュール表の親行にトップレベル項目ごとの薄い青系配色を追加

**既知の課題・未実装事項**:
- 実データでの最終的な見た目確認(特に5-E/5-F/5-Gの組み合わせ)は、ヘッドレスChromeでの
  IndexedDB投入テストを断念したため、ユーザーによるブラウザでの手動確認が必要
- メモ表示切替タブ(編集/プレビュー/分割)とマインドマップのカラースウォッチは、5-Bで意図的に
  アイコン化対象から外しており、docs/rules上の統一デザインからは外れたまま残っている
- 5-F実装時、ヘッダーの土日祝の色分け判定(`isSundayOrHoliday`/`isSaturday`)は新設した
  `isBusinessDay`と重複したロジックのまま残しており、共通化はしていない(色分岐の要件が異なるため)
