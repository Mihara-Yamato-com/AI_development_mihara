...
3-F作成時点では3-E（マインドマップ、`mindMapNodes`）がdevelop未マージだったため、`mindMapNodes`はスナップショット対象に含めていない。3-Eとの統合時に、`data.mindMapNodes`と、`clearProjectData`・`restoreSnapshotById`での該当処理を追加すること（3-Fの初期実装では誤って未定義の`getMindMapNodesByProject`等を参照しており、保存ボタンが無反応になる不具合が発生したため、一旦除去した経緯がある）。

**復元の仕組み**: 復元時は`clearProjectData`で現在のプロジェクトに紐づく全データを削除してから、スナップショットの内容を`insertRawRecord`で再投入する。`scheduleItems`は自己参照の`parentId`を持つため、再投入時に旧id→新idの対応表を作り、2周目で`parentId`を新しいidに付け替える。`comments`も同様に`scheduleItemId`を新idへ付け替えてから投入する。
